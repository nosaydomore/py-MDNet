...
def train_mdnet():
    ...
    for i in range(opts['n_cycles']):
        ...
        for j,k in enumerate(k_list):
            ...
            pos_score = model(pos_regions, k) # 将从训练集的正负样本放入网络模型中得到score
            neg_score = model(neg_regions, k)

            loss = criterion(pos_score, neg_score)
            model.zero_grad() # 清空梯度值,防止上次的梯度累计
            loss.backward()
        ...
        cur_prec = prec.mean()
        print "Mean Precision: %.3f" % (cur_prec) # K个场景的平均准确率
...
if __name__ == "__main__": # 在if __name__ == 'main': 下的代码只有在第一种情况下（即文件作为脚本直接执行）才会被执行，而import到其他脚本中是不会被执行的
    train_mdnet()
