...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    print 'Init criterion and optimizer over'
    tic = time.time()
    # Load first image
    image = Image.open(img_list[0]).convert('RGB')
    print 'Load first image over'
    # Train bbox regressor
    bbreg_examples = gen_samples(SampleGenerator('uniform', image.size, 0.3, 1.5, 1.1), # 0.3为位移因子,1.5为尺度因子,1.1为长宽比
                                 target_bbox, opts['n_bbreg'], opts['overlap_bbreg'], opts['scale_bbreg'])
    bbreg_feats = forward_samples(model, image, bbreg_examples)
    bbreg = BBRegressor(image.size)
    bbreg.train(bbreg_feats, bbreg_examples, target_bbox)
    print 'Train bbox regressor over'
    ...
    neg_examples = np.concatenate([
                    ...
                    gen_samples(SampleGenerator('whole', image.size, 0, 1.2, 1.1),
                                target_bbox, opts['n_neg_init']//2, opts['overlap_neg_init'])]) # 数组拼接
    neg_examples = np.random.permutation(neg_examples)
    print 'Draw pos/neg samples over'
    # Extract pos/neg features
    pos_feats = forward_samples(model, image, pos_examples)
    neg_feats = forward_samples(model, image, neg_examples)
    feat_dim = pos_feats.size(-1)
    print 'Extract pos/neg features over'
    # Initial training
    ...
    print 'Init sample generators over'
    # Init pos/neg features for update
    pos_feats_all = [pos_feats[:opts['n_pos_update']]]
    neg_feats_all = [neg_feats[:opts['n_neg_update']]]
    print 'Init pos/neg features for update over'
...
if __name__ == "__main__":
    ...
    print 'Generate sequence config over'
